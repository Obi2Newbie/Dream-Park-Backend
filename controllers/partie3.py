...
def creer_base_donnees():
    ...
    # IMPORTANT: L'ordre est important à cause des clés étrangères!
    tables = [
    ...
    conn.commit()
    print("\n🗄️  Création de la base de données DreamPark...")
    ...
    cursor.execute('''
                   ...
                   ''')
    print("   ✓ Table Statistiques_Frequentation créée")

    conn.commit()
    print("\n✅ Structure de la base de données créée avec succès!\n")
...
def peupler_base_donnees(conn):
    ...
    cursor.execute('UPDATE Parking SET nb_places_libres = 199 WHERE id_parking = ?', (id_parking,))
    ...
    cursor.execute('''
                   ...
                   VALUES (?, NULL, 'Maintenance non effectuée')
                   ''', (date.today(),))
    ...
    cursor.execute('''
                   ...
                   VALUES (?, ?, ?, ?)
                   ''', (id_service_liv, 'Something Street', '18:00', clients[1]))
    ...
    for jour in range(30):
        ...
        cursor.execute('''
                       INSERT INTO Statistiques_Frequentation (date_stat, nb_entrees, nb_sorties, taux_occupation,
                                                               revenue_journalier, id_parking)
                       VALUES (?, ?, ?, ?, ?, ?)
...
def main():
    ...
    try:
        ...
        print("   Vous pouvez maintenant utiliser partie4.py\n")
